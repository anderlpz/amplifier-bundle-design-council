...
version: 1.0.0
description: |
  Intent reviewer for any design checkpoint: a design director asking "what is
  this for?" of every element.
  Hunts decoration that does no work — the animation that says nothing, the
  space-filling illustration.
  Use when: elements are present without a stated reason, decoration outweighs
  communication, or nobody can say what a choice is meant to say — any time
  the worry is "why is this here, and what is it saying?"
user-invocable: true
shortcut: PK
model_role: critique
---
